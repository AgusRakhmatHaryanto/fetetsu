...
export default function NewPaymentProof() {
  ...
  const handleAddPaymentProof = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8081/api/v1/payment-proof', formData);
      if (response.status !== 200) {
        throw new Error('Gagal menambahkan bukti pembayaran');
      }

      await axios.put(`http://localhost:8081/api/v1/orders/${newPaymentProof.orderId}`, {
        paymentStatus: 'PAYED'
      });
