...
const OrderItemProgress = ({ params }) => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:8081/api/v1/progress/order-item/${orderItemId}`);
  ...
  const handleAddProgress = async () => {
    try {
      const response = await axios.post(`http://localhost:8081/api/v1/progress/order-item/${orderItemId}`, newProgress);
  ...
  const handleEditProgress = async (id) => {
    try {
      const response = await axios.put(`http://localhost:8081/api/v1/progress/${id}`, { description: editingProgress.description });
  ...
  const handleDeleteProgress = async (id) => {
    try {
      await axios.delete(`http://localhost:8081/api/v1/progress/${id}`);
