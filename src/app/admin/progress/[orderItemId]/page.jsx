...
const OrderItemProgress = ({ params }) => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${process.env.API_URL}progress/order-item/${orderItemId}`);
  ...
  const handleAddProgress = async () => {
    try {
      const response = await axios.post(`${process.env.API_URL}progress/order-item/${orderItemId}`, newProgress);
  ...
  const handleEditProgress = async (id) => {
    try {
      const response = await axios.put(`${process.env.API_URL}progress/${id}`, { description: editingProgress.description });
  ...
  const handleDeleteProgress = async (id) => {
    try {
      await axios.delete(`${process.env.API_URL}progress/${id}`);
